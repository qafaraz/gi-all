...
- **Никакого ручного merge** – Выбираете стэк (например, Angular + Node + Android + Unity + Docker + VS Code), а `gi-all`:
  ...
  - добавляет обязательные правила безопасности для типовых secret- и credential-файлов.
...
- Файлы credential и облачных настроек: `.envrc`, `.npmrc`, `.netrc`, `.aws/`, `credentials.json`
- Инфраструктурные и мобильные секреты: `*.tfstate`, `*.tfvars`, `*.tfplan`, `*.mobileprovision`, `GoogleService-Info.plist`
- Хранилища секретов: `secrets.*`, `*.kdbx`, `serviceAccountKey.json`, `firebase-adminsdk*.json`
...
Даже если какой‑то шаблон устарел или неполон, `gi-all` всё равно добавляет базовый слой защиты.

> `gi-all` заметно снижает риск случайного коммита типовых secret-файлов, но проектно-специфичные credential-файлы всё равно стоит проверять вручную.
...
- **Конфликты**:
  ...
  - Для безопасности `gi-all` отказывается перезаписывать `.gitignore`, если цель является символической ссылкой или файлом с несколькими hardlink.

---

## 📦 Установка

Требуется Node.js `>=22.0.0` (рекомендуется Node 22 или 24 LTS).
